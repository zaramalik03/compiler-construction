 ...
 */
 ...
 void initialize(){
    ...
    // table.entry = (struct symbolTableEntry *)malloc(table.capacity * sizeof(struct symbolTableEntry));
     table.entry = malloc(table.capacity * sizeof(struct symbolTableEntry));
 ...
 void insert(char *name, char data_type, char kind_of_variable, int size_of_variable) {
     ...
     if (kind_of_variable == VK_ARRAY) {
         new_entry->address_of_variable = table.next_address;
         table.next_address += size_of_variable;
     } else {
         new_entry->address_of_variable = table.next_address++;
     }    
 ...
 int generateISP() {
     int total_size = 0;
     for (int i = 0; i < table.count; i++) {
         total_size += table.entry[i].size_of_variable;
     }
     return total_size;
 }
